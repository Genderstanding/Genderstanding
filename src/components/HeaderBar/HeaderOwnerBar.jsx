...
import InviteNodeModal from "../AddNodeModal/AddNodeModal";
...
export default function HeaderOwnerBar({ newNodeData }) {
  ...
  <div style={{margin:"25px"}}><Typography >{newNode?.[0]?.node_name || newNode?.node_name}</Typography></div>
      {/* this flex-grow div is tailwind way to spread out the back and add buttons*/}
      <div className="flex-grow"></div>
      <button className="mr-4 text-2xl" onClick={openInviteCode}>
        <SVG
          width={24}
          height={24}
          viewBox="8 5 27 27"
